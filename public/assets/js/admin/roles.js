...
async function loadRoles(page = 1) {
    ...
    try {
        ...
        tbody.innerHTML = json.data
            .map((role, index) => {
                const permBadges = role.permissions.length
                    ? role.permissions
                          .map(
                              (p) => `<span class="green-btn">${p.name}</span>`,
                          )
                          .join("")
        ...
        console.error(err);
        tbody.innerHTML = `<tr><td colspan="5" class="text-center text-danger py-4">Failed to load roles.</td></tr>`;
...
document.getElementById("createRoleBtn").addEventListener("click", async () => {
    ...
    try {
        ...
        if (!json.status) {
            if (json.errors?.name)
                setFieldError("createRoleName", json.errors.name[0]);
...
document.getElementById("editRoleBtn").addEventListener("click", async () => {
    ...
    try {
        ...
        if (!json.status) {
            if (json.errors?.name)
                setFieldError("editRoleNameInput", json.errors.name[0]);
            else showToast(json.message, "danger");
            return;
        }
