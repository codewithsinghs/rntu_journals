...
@section('content')
    ...
    {{-- Edit ID --}}
    <div class="p_inner d-none" id="saEditPage" data-id="{{ $id }}">
        <div id="saEditSubtitle"></div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function showToast(type, title, msg) {
                ...
                iconEl.className = type === 'success' ? 'bi bi-check-circle' : 'bi bi-exclamation-circle';
            ...
            async function loadIssuesForJournal(journalId, selectedId) {
                ...
                try {
                    ...
                    const json = await res.json();
                    if (!res.ok || !json.status) throw new Error(json.message || 'Failed to load issues.');
                    const list = json.data || [];
                    select.innerHTML = '<option value="">Auto (use current issue)</option>' +
                        list.map(i => `<option value="${i.id}" ${String(selectedId) === String(i.id) ? 'selected' : ''}>
            ...
            async function load() {
                try {
                    ...
                    keywordTags = Array.isArray(json.data.keywords) ? [...json.data.keywords] : [];
            ...
            function renderActions(r) {
                ...
                if (r.can_approve) {
                    document.getElementById('saApproveBtn').addEventListener('click', doApprove);
                }
                if (r.can_reject) {
                    document.getElementById('saRejectBtn').addEventListener('click', openReject);
                }
                if (r.can_forward) {
                    document.getElementById('saForwardBtn').addEventListener('click', openForward);
                }
            }
            ...
            document.getElementById('saForwardConfirmBtn').addEventListener('click', async () => {
                ...
                try {
                    const res = await fetch(`${API_BASE}/${id}/forward-to-reviewer`, {
                        method: 'POST',
                        headers: {
                            ...authHeaders(),
                            'Content-Type': 'application/json'
            ...
            function renderKeywordTags() {
                ...
                const chips = keywordTags.map((kw, idx) => `
        ...
        `;
                ...
                textInput.addEventListener('keydown', (e) => {
                    ...
                    } else if (e.key === 'Backspace' && textInput.value === '' && keywordTags.length) {
                        keywordTags.pop();
                        renderKeywordTags();
                    }
                });
            ...
            function renderCoAuthors() {
                ...
                wrap.innerHTML = coAuthors.map((c, idx) => `
        ...
        `).join('') || '<p class="mb-0">No co-authors added yet.</p>';
            ...
            function renderReviewers() {
                ...
                wrap.innerHTML = reviewersList.map((r, idx) => `
        ...
        `).join('') || '<p class="mb-0">No reviewers added yet.</p>';
                ...
                wrap.querySelectorAll('[data-remove-reviewer]').forEach(btn => {
                    btn.addEventListener('click', () => {
                        reviewersList.splice(parseInt(btn.dataset.removeReviewer, 10), 1);
                        renderReviewers();
                    });
                });
            }
            ...
            function render(r) {
                ...
                const revisionBanner = (currentStage === 'with_author') ? `
                ...
                const declChecks = Object.keys(declLabels).map(key => {
                    const checked = (r.declarations || []).includes(key) ? 'checked' : '';
                    return `
        ...
        `;
                ...
                if (r.can_edit_issue) {
                    ...
                    document.getElementById('edit_journal_id').addEventListener('change', (e) => {
                ...
                document.getElementById('saEditForm').addEventListener('submit', function(e) {
                    e.preventDefault();
                    save();
                });
            }
            ...
            function collectCoAuthorsFromDOM() {
                const rows = document.querySelectorAll('#edit_co_authors_wrap .sa-dyn-row');
                return Array.from(rows).map(row => ({
                    name: row.querySelector('.sa-coauthor-name').value,
                    email: row.querySelector('.sa-coauthor-email').value,
                    affiliation: row.querySelector('.sa-coauthor-affiliation').value,
                    orcid_id: row.querySelector('.sa-coauthor-orcid').value,
                })).filter(c => c.name.trim() || c.email.trim());
            }

            function collectReviewersFromDOM() {
                const rows = document.querySelectorAll('#edit_reviewers_wrap .sa-dyn-row');
                return Array.from(rows).map(row => ({
                    name: row.querySelector('.sa-reviewer-name').value,
                    email: row.querySelector('.sa-reviewer-email').value,
                    institution: row.querySelector('.sa-reviewer-institution').value,
                    area_of_expertise: row.querySelector('.sa-reviewer-expertise').value,
            ...
            async function save() {
                ...
                const declarations = Array.from(document.querySelectorAll('.sa-decl-check:checked')).map(cb =>
                    cb.value);
                ...
                fd.append('journal_id', document.getElementById('edit_journal_id').value);
                ...
                fd.append('affiliating_institute', document.getElementById('edit_affiliating_institute').value);
                fd.append('department', document.getElementById('edit_department').value);
                fd.append('affiliating_institute_address', document.getElementById(
                    'edit_affiliating_institute_address').value);
                fd.append('manuscript_title', document.getElementById('edit_manuscript_title').value);
                fd.append('abstract_summary', document.getElementById('edit_abstract_summary').value);
                fd.append('references', document.getElementById('edit_references').value);
                fd.append('author_signature', document.getElementById('edit_author_signature').value);
                ...
                finalReviewers.forEach((r, i) => {
                    ...
                    fd.append(`reviewers[${i}][area_of_expertise]`, r.area_of_expertise);
                });
                if (finalReviewers.length === 0) fd.append('reviewers', '');

                const manuscriptFile = document.getElementById('edit_signed_manuscript_pdf').files[0];
                const abstractFile = document.getElementById('edit_abstract_file').files[0];
                const signatureFile = document.getElementById('edit_signature_file').files[0];
                ...
                try {
                    ...
                    if (currentStage === 'with_author') {
                        ...
                        if (!resubmitRes.ok || !resubmitJson.status) throw new Error(resubmitJson.message ||
                            'Resubmit failed.');
